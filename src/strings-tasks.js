...
function repeatString(str, times) {
  return Boolean(times) === true && times > 0 ? str.repeat(times) : '';
}
...
function removeLastOccurrences(str, value) {
  ...
  return lastI >= 0
    ? str.substring(0, lastI) + str.substr(lastI + value.length)
    : str;
}
...
function sumOfCodes(str) {
  ...
  return str
    ...
    .reduce((prVal, item) => prVal + item, 0);
}
...
function isPalindrome(str) {
  ...
  const text = str
    .toUpperCase()
    .split('')
    .filter((item) => !specSymbols.includes(item))
    .join('');
...
function reverseWords(str) {
  const row = str.split(' ').map((item) => item.split(''));
  return row.map((item) => item.reverse().join('')).join(' ');
}
